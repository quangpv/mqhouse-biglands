...
async def report_sold_out(
    ...
    if listing.status != ListingStatus.CON_HANG:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Listing must be in CON_HANG to report sold out")
    ...
    for approver in approvers:
        await send_notification(
            db=db, user_id=approver.id, event_type="sold_out_reported",
