  ...
  return (
    <div className="space-y-2 rounded-lg border p-4 text-sm">
      <div className="flex items-center justify-between">
        ...
        <span className="font-medium">{listing.creator?.full_name ?? "—"}</span>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-muted-foreground">Liên hệ:</span>
        <span className="font-medium">{formatPhone(listing.creator?.phone ?? null) || "—"}</span>
      </div>
      <div className="flex items-center justify-between">
        ...
        <button onClick={onToggleOwnerPhone} className="flex items-center gap-1 font-medium hover:underline">
          {ownerPhoneVisible ? (
            <>{formatPhone(listing.owner_phone)} <EyeOff className="h-3.5 w-3.5 text-muted-foreground" /></>
          ) : (
            <>{listing.owner_phone ? "Hiện số" : "—"} <Eye className="h-3.5 w-3.5 text-muted-foreground" /></>
          )}
        </button>
      </div>
      <div className="flex items-center justify-between">
