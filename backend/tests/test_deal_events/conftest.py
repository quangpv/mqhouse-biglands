...
async def con_hang_agent_listing(db_session: AsyncSession) -> str:
...
@pytest_asyncio.fixture
async def draft_agent_listing(db_session: AsyncSession) -> str:
...
def _make_listing(owner_id, status):
    return ListingEntity(
        ...
        description=f"Test listing for deal events {uuid.uuid4().hex[:8]}",
