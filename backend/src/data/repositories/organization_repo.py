import uuid

from fastapi import Depends
from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from src.data.entities.organization import OrganizationEntity
from src.platform.dependencies import get_db


class OrganizationRepo:
    def __init__(self, db: AsyncSession = Depends(get_db)):
        self.db = db

    async def get(self, org_id: uuid.UUID) -> OrganizationEntity | None:
        result = await self.db.execute(
            select(OrganizationEntity).where(OrganizationEntity.id == org_id)
        )
        return result.scalar_one_or_none()

    async def get_by_name(self, name: str) -> OrganizationEntity | None:
        result = await self.db.execute(
            select(OrganizationEntity).where(OrganizationEntity.name == name)
        )
        return result.scalar_one_or_none()

    async def get_all(self) -> list[OrganizationEntity]:
        result = await self.db.execute(
            select(OrganizationEntity).order_by(OrganizationEntity.name)
        )
        return list(result.scalars().all())

    async def save(self, entity: OrganizationEntity) -> OrganizationEntity:
        self.db.add(entity)
        await self.db.flush()
        await self.db.refresh(entity)
        return entity

    async def delete(self, entity: OrganizationEntity) -> None:
        await self.db.delete(entity)
        await self.db.flush()
