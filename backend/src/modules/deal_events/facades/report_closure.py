...
async def report_closure(
    ...
    if listing.status != ListingStatus.DA_COC:
        raise ConflictError("Listing must be in DA_COC to report a closure")
    ...
    if current_user.role == UserRole.ADMIN:
        ...
        if listing.created_by_id != current_user.id:
            await notification_service.send(
                user_id=listing.created_by_id, event_type="closure_confirmed",
                ...
                body=f"Closure has been confirmed by admin {current_user.full_name}.",
        ...
        for approver in approvers:
            await notification_service.send(
                user_id=approver.id, event_type="closure_reported",
                title=format_notification_title(
                ...
                body=f"A closure has been reported by {current_user.full_name}.",
