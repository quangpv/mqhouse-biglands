...
@router.get("/", response_model=TransactionTypeListResponse)
...
@router.get("/{entity_id}", response_model=TransactionTypeResponse)
async def get_endpoint(result: TransactionTypeResponse = Depends(get_transaction_type)):
    return result


@router.put("/{entity_id}", response_model=TransactionTypeResponse)
