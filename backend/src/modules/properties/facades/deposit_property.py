...
from src.modules.properties.schemas import DepositRequest, PropertyResponse
from src.platform.auth import get_current_user
from src.shared.errors.exceptions import ForbiddenError, NotFoundError


async def deposit_property(
    body: DepositRequest,
    ...
    if entity.status != PropertyStatus.AVAILABLE:
        raise ForbiddenError("Only available properties can be deposited")
    ...
    if current_user.role == UserRole.SALE:
        new_status = PropertyStatus.DEPOSIT_PENDING
    elif current_user.role in (UserRole.ADMIN, UserRole.APPROVER):
        new_status = PropertyStatus.DEPOSITED
    else:
